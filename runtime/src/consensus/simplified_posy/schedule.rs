...
pub struct SimplifiedEpochContext {
    ...
    /// Present for every Genesis-activated production v3 context. Older
    /// synthetic test/harness contexts use the compatibility constructor and
    /// are rejected by activation validation before production startup.
    #[serde(default)]
...
impl SimplifiedEpochContext {
    ...
    pub fn validate_against(&self, validator_set: &ValidatorSet) -> Result<(), String> {
        ...
        if self.leader_ring.len() != active_set.validators.len() {
            return Err("simplified leader ring size does not match the frozen epoch set".to_string());
        }
