    ...
    fn observe(
        ...
        match item {
            SimplifiedStateSyncEvidence::QuorumCertificate(qc) => {
                ...
                let crossed_lease_boundary = if self.next_height
                    <= epoch_context.epoch_end_height.0
                {
...
fn validate_export_envelope(bundle: &SimplifiedStateSyncBundle) -> Result<(), String> {
    ...
    if bundle.claimed_finalized.block_id.0.trim().is_empty()
        ...
        || bundle.claimed_finalized.height.0 < bundle.anchor_qc.height.0
    {
    ...
    if !finalized_is_anchor && !finalized_is_supplied_qc {
...
pub fn build_state_sync_chunks(
    ...
    for item in evidence {
        let item_len = item.canonical_bytes()?.len();
        if item_len > MAX_SIMPLIFIED_STATE_SYNC_CHUNK_PAYLOAD_BYTES / 2 {
        ...
        let current_len = current.iter().try_fold(0usize, |sum, value| {
            sum.checked_add(value.canonical_bytes()?.len())
                .ok_or_else(|| "state-sync chunk size overflows".to_string())
        })?;
        let projected_len = current_len
            .checked_add(item_len)
            .ok_or_else(|| "state-sync chunk size overflows".to_string())?;
        if !current.is_empty() && projected_len > MAX_SIMPLIFIED_STATE_SYNC_CHUNK_PAYLOAD_BYTES / 2
        {
...
impl SimplifiedStateSyncStager {
    ...
    pub fn accept(
        ...
        if !self.active.contains_key(&key) {
            ...
            self.active.insert(key.clone(), StagedSession {
                request_id: chunk.request_id,
        ...
        self.completed
            .push_back((peer.clone(), completed.request_id, completed.session_root, replay_expires_at));
...
mod tests {
    ...
    use crate::synergy_types::{
        AegisPqKeyId, AegisPqPublicKey, BlockId, ClusterId, Epoch, Height, UmaId, ValidatorRecord,
        ValidatorSet, ValidatorStatus, TESTNET_V3_CONSENSUS_SIGNATURE_ALGORITHM,
    ...
    fn two_chunk_stream() -> (
        ...
        let session_root = session_root(context_root, &anchor, &evidence, &anchor_finalized())
    ...
    fn empty_bundle_should_round_trip_through_one_final_chunk() {
        ...
        let chunks = build_state_sync_chunks(&bundle).expect("empty bundle should chunk");
    ...
    fn takeover_authority_persists_across_certified_heights_in_one_lease() {
        ...
        let timeout = SimplifiedTimeoutCertificate {
            ...
            lease_index: context.lease_index(crate::synergy_types::Height(1_000)).unwrap(),
        ...
        let chunks = build_state_sync_chunks(&bundle).unwrap();
    ...
    fn final_claim_tamper_should_fail_the_session_transcript() {
        let bundle = empty_bundle();
        let mut chunk = build_state_sync_chunks(&bundle)
    ...
    fn completed_session_should_reject_immediate_replay() {
        let chunk = build_state_sync_chunks(&empty_bundle())
            .expect("empty bundle should chunk")
            .remove(0);
        let mut stager = SimplifiedStateSyncStager::new(epoch_context(), anchor_qc())
            .expect("stager should initialize");
    ...
    fn non_final_empty_chunk_should_be_rejected() {
        let mut chunk = build_state_sync_chunks(&empty_bundle())
            .expect("empty bundle should chunk")
            .remove(0);
    ...
    fn active_session_should_remain_bound_to_its_first_peer() {
        ...
        assert!(error.contains("peer or immutable identity"), "{error}");
    }
    ...
    fn exporter_should_reject_a_qc_that_skips_the_next_height() {
        ...
        let error = build_state_sync_chunks(&bundle)
    ...
    fn self_consistent_session_should_reject_an_unanchored_finalized_claim() {
        ...
        let session_root = session_root(context_root, &bundle.anchor_qc, &[], &bad_finalized)
            .expect("test session should hash");
        let mut chunk = SimplifiedStateSyncChunk {
            format: POSY_SIMPLIFIED_STATE_SYNC_CHUNK_FORMAT.to_string(),
            session_root,
