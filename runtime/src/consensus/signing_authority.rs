...
impl DurableConsensusSigningAuthority {
    ...
    /// Returns the complete validated authorization history.
    ///
    /// This is a read-only restart-reconciliation surface. Callers must still
    /// scope records to their exact chain, epoch, context, validator, and key;
    /// no returned record grants new signing authority.
    ...
    fn load_unlocked(&self) -> Result<DurableSigningJournal, String> {
        ...
        for record in &journal.records {
            ...
            let expected_slot = record.authorization.slot_key();
            if record.slot != expected_slot {
                return Err("consensus signing journal slot mismatch".to_string());
            }
            ...
            if record.authorization_root != expected_root {
                return Err("consensus signing journal authorization root mismatch".to_string());
            }
