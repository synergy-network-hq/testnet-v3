//! Signs one exact Chain 1266 start command. The verification authority is
//! already pinned by the attested desired-state manifest.
...
use synergy_testnet::consensus_start::{
    Chain1266StartRequest, SignedChain1266StartCommand, CHAIN1266_START_SIGNATURE_DOMAIN,
};
...
fn fail(message: impl AsRef<str>) -> ! {
    eprintln!("sign-chain1266-start-command: {}", message.as_ref());
    std::process::exit(1);
}
...
fn main() {
    ...
    let activate_unix_ms = arg(&args, "--activate-unix-ms")
        .parse::<u64>()
        .unwrap_or_else(|error| fail(format!("invalid activation time: {error}")));
    ...
    let request = Chain1266StartRequest {
        schema_version: 1,
        action: "START_CONSENSUS".to_string(),
        ...
        signature_domain: CHAIN1266_START_SIGNATURE_DOMAIN.to_string(),
        authority_public_key_fingerprint: desired.start_authority.public_key_fingerprint,
    };
    let canonical = serde_json::to_vec(&request)
        .unwrap_or_else(|error| fail(format!("encode canonical start request: {error}")));
    let signature = Sign::mldsa87()
        ...
        .unwrap_or_else(|error| fail(format!("sign ML-DSA-87 start command: {error}")));
    let command = SignedChain1266StartCommand {
        request,
    ...
    let mut encoded = serde_json::to_vec_pretty(&command)
        .unwrap_or_else(|error| fail(format!("encode signed start command: {error}")));
    encoded.push(b'\n');
    fs::write(&output_path, encoded)
        .unwrap_or_else(|error| fail(format!("write {}: {error}", output_path.display())));
    println!("CHAIN1266_START_COMMAND_SIGNED {}", output_path.display());
}
