...
use crate::synergy_types::{
    ...
    POSY_PROTOCOL_VERSION, SYNERGY_TESTNET_V3_CHAIN_ID, SYNERGY_TESTNET_V3_NETWORK_ID,
};
use crate::telemetry;
    ...
    match subcommand.as_str() {
        ...
        "start" => {
                    ...
                    match select_finalized_consensus_driver_startup(
                        ...
                        Ok(FinalizedConsensusDriverStartup::SpawnSimplifiedV3Driver {
                            ...
                            let network = match p2p_network.as_ref().cloned() {
                                ...
                                None => {
                                    eprintln!(
                                        "Consensus activation failed closed: simplified PoSy requires an active P2P network"
                                    );
                                    process::exit(1);
                                }
                            };
                            typed_posy_worker = match spawn_finalized_simplified_posy_driver(
                                &config,
                                epoch_context,
                                validator_set,
                                network,
            ...
            let typed_worker_failure = typed_posy_worker
                .as_ref()
                .and_then(FinalizedPosyWorker::fatal_error);
