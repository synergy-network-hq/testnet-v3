...
pub fn sign(msg: &[u8], sk: &SecretKey) -> Result<SignedMessage> {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_signature!(PQCLEAN_MLDSA44_AVX2_crypto_sign_ctx, msg, sk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return gen_signature!(PQCLEAN_MLDSA44_AARCH64_crypto_sign_ctx, msg, sk);
        }
    }
    gen_signature!(PQCLEAN_MLDSA44_CLEAN_crypto_sign_ctx, msg, sk)
}
...
pub fn open(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return open_signed!(PQCLEAN_MLDSA44_AVX2_crypto_sign_open_ctx, sm, pk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return open_signed!(PQCLEAN_MLDSA44_AARCH64_crypto_sign_open_ctx, sm, pk);
        }
    }
    open_signed!(PQCLEAN_MLDSA44_CLEAN_crypto_sign_open_ctx, sm, pk)
}
...
pub fn detached_sign(msg: &[u8], sk: &SecretKey) -> Result<DetachedSignature> {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return detached_signature!(PQCLEAN_MLDSA44_AVX2_crypto_sign_signature_ctx, msg, sk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return detached_signature!(PQCLEAN_MLDSA44_AARCH64_crypto_sign_signature_ctx, msg, sk);
        }
    }
    detached_signature!(PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature_ctx, msg, sk)
}
...
pub fn verify_detached_signature(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return verify_detached_sig!(PQCLEAN_MLDSA44_AVX2_crypto_sign_verify_ctx, sig, msg, pk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
    ...
    verify_detached_sig!(PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify_ctx, sig, msg, pk)
}
...
mod test {
    ...
    pub fn test_sign() {
        ...
        let (pk, sk) = keypair().expect("ML-DSA-44 keypair should succeed");
        let sm = sign(&message, &sk).expect("ML-DSA-44 signing should succeed");
    ...
    pub fn test_sign_detached() {
        ...
        let (pk, sk) = keypair().expect("ML-DSA-44 keypair should succeed");
        let sig = detached_sign(&message, &sk).expect("ML-DSA-44 detached sign should succeed");
    ...
    pub fn test_sign_ctx() {
        ...
        let message = (0..len).map(|_| rng.random::<u8>()).collect::<Vec<_>>();
        let (pk, sk) = keypair().expect("ML-DSA-44 keypair should succeed");
        let sm = sign_ctx(&message, &ctx, &sk).expect("ML-DSA-44 contextual sign should succeed");
    ...
    pub fn test_sign_detached_ctx() {
        ...
        let sig = detached_sign_ctx(&message, &ctx, &sk)
            .expect("ML-DSA-44 contextual detached sign should succeed");
