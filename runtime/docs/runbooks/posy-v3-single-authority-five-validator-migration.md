...
9. For the currently integrated initial runtime profile, prove that finalized
   ETDAG activation is deferred; the role runtime must refuse startup if an
   active ETDAG permit is present. Treat ETDAG/BOC/reveal-derived protected
   execution as a future full-profile gate until the production protected
   proposal-material adapter is installed and qualified. That future path may
   delay proposals but must never create leader authority or lower quorum.
...
- At the boundary: accept the finalized transition certificate, atomically
  install the v3 context, restore/initialize durable v3 safety state, and
  require the scheduled owner from the committed ring. The currently integrated
  initial path additionally requires finalized ETDAG activation to remain
  deferred. No node may infer the boundary from wall time.
