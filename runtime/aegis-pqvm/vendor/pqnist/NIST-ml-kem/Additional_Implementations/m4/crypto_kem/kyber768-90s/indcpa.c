...
void indcpa_keypair(unsigned char *pk, unsigned char *sk) {
    ...
    unsigned char *noiseseed = buf + MLKEM_SYMBYTES;
    int i;
    ...
    memcpy(pk + MLKEM_POLYVECBYTES, publicseed, MLKEM_SYMBYTES); // Pack the public seed in the public key
}
...
*
*
* Arguments:   - unsigned char *ct:         pointer to input ciphertext to compare the new ciphertext with (of length MLKEM_INDCPA_BYTES bytes)
*              - const unsigned char *m:    pointer to input message (of length MLKEM_INDCPA_MSGBYTES bytes)
*              - const unsigned char *pk:   pointer to input public key (of length MLKEM_INDCPA_PUBLICKEYBYTES bytes)
*              - const unsigned char *coin: pointer to input random coins used as seed (of length MLKEM_SYMBYTES bytes)
*                                           to deterministically generate all randomness
...
void __attribute__ ((noinline)) indcpa_dec(unsigned char *m,
    ...
    for(int i = 1; i < MLKEM_K; i++) {
        ...
        poly_ntt(&bp);
        poly_frombytes_mul(&bp, sk + i*MLKEM_POLYBYTES);
        poly_add(&mp, &mp, &bp);
    }
