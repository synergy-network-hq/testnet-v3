...
//! # Example
//! ```
//! use crate::pqc::signatures::fndsa::fndsa512::*;
...
use super::ffi;
use alloc::vec::Vec;
use pqrust_traits::sign as primitive;
...
simple_struct!(PublicKey, ffi::PQCLEAN_FNDSA512_CLEAN_CRYPTO_PUBLICKEYBYTES);
simple_struct!(SecretKey, ffi::PQCLEAN_FNDSA512_CLEAN_CRYPTO_SECRETKEYBYTES);

#[derive(Clone, Copy)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct DetachedSignature(
    #[cfg_attr(feature = "serialization", serde(with = "BigArray"))]
    [u8; ffi::PQCLEAN_FNDSA512_CLEAN_CRYPTO_BYTES],
    usize,
);
...
impl DetachedSignature {
    fn new() -> Self {
        DetachedSignature([0u8; ffi::PQCLEAN_FNDSA512_CLEAN_CRYPTO_BYTES], 0)
    }
}
...
impl primitive::DetachedSignature for DetachedSignature {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let expected = ffi::PQCLEAN_FNDSA512_CLEAN_CRYPTO_BYTES;
        if actual > expected {
        ...
        let mut array = [0u8; ffi::PQCLEAN_FNDSA512_CLEAN_CRYPTO_BYTES];
...
/// Generate a FN-DSA-512 keypair
pub fn keypair() -> (PublicKey, SecretKey) {
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair!(PQCLEAN_FNDSA512_AVX2_crypto_sign_keypair);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        ...
        // the moment
        if true {
            return gen_keypair!(PQCLEAN_FNDSA512_AARCH64_crypto_sign_keypair);
        }
    }
    gen_keypair!(PQCLEAN_FNDSA512_CLEAN_crypto_sign_keypair)
}
...
pub fn sign(msg: &[u8], sk: &SecretKey) -> SignedMessage {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_signature!(PQCLEAN_FNDSA512_AVX2_crypto_sign, msg, sk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return gen_signature!(PQCLEAN_FNDSA512_AARCH64_crypto_sign, msg, sk);
        }
    }
    gen_signature!(PQCLEAN_FNDSA512_CLEAN_crypto_sign, msg, sk)
}
...
pub fn open(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return open_signed!(PQCLEAN_FNDSA512_AVX2_crypto_sign_open, sm, pk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return open_signed!(PQCLEAN_FNDSA512_AARCH64_crypto_sign_open, sm, pk);
        }
    }
    open_signed!(PQCLEAN_FNDSA512_CLEAN_crypto_sign_open, sm, pk)
}
...
pub fn detached_sign(msg: &[u8], sk: &SecretKey) -> DetachedSignature {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return detached_signature!(PQCLEAN_FNDSA512_AVX2_crypto_sign_signature, msg, sk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return detached_signature!(PQCLEAN_FNDSA512_AARCH64_crypto_sign_signature, msg, sk);
        }
    }
    detached_signature!(PQCLEAN_FNDSA512_CLEAN_crypto_sign_signature, msg, sk)
}
...
pub fn verify_detached_signature(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return verify_detached_sig!(PQCLEAN_FNDSA512_AVX2_crypto_sign_verify, sig, msg, pk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return verify_detached_sig!(PQCLEAN_FNDSA512_AARCH64_crypto_sign_verify, sig, msg, pk);
        }
    }
    verify_detached_sig!(PQCLEAN_FNDSA512_CLEAN_crypto_sign_verify, sig, msg, pk)
}
...
mod test {
    ...
    pub fn test_sign_detached() {
        ...
        assert!(verify_detached_signature(&sig, &message[..message.len() - 1], &pk).is_err());
    }
}
