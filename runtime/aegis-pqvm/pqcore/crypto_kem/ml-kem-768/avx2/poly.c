...
/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_compress
*
...
*              by PQCLEAN_MLKEM768_AVX2_poly_reduce().
*
* Arguments:   - uint8_t *r: pointer to output byte array
*                            (of length MLKEM_POLYCOMPRESSEDBYTES)
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_compress(uint8_t r[128], const poly *restrict a) {
    unsigned int i;
    __m256i f0, f1, f2, f3;
    const __m256i v = _mm256_load_si256(&PQCLEAN_MLKEM768_AVX2_qdata.vec[_16XV / 16]);
...
void PQCLEAN_MLKEM768_AVX2_poly_decompress(poly *restrict r, const uint8_t a[128]) {
    unsigned int i;
    __m128i t;
    __m256i f;
    const __m256i q = _mm256_load_si256(&PQCLEAN_MLKEM768_AVX2_qdata.vec[_16XQ / 16]);
...
/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_tobytes
*
* Description: Serialization of a polynomial in NTT representation.
*              The coefficients of the input polynomial are assumed to
*              lie in the interval [0,q], i.e. the polynomial must be reduced
*              by PQCLEAN_MLKEM768_AVX2_poly_reduce(). The coefficients are orderd as output by
*              PQCLEAN_MLKEM768_AVX2_poly_ntt(); the serialized output coefficients are in bitreversed
*              order.
*
* Arguments:   - uint8_t *r: pointer to output byte array
...
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_tobytes(uint8_t r[MLKEM_POLYBYTES], const poly *a) {
    PQCLEAN_MLKEM768_AVX2_ntttobytes_avx(r, a->vec, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_frombytes
*
* Description: De-serialization of a polynomial;
*              inverse of PQCLEAN_MLKEM768_AVX2_poly_tobytes
*
* Arguments:   - poly *r: pointer to output polynomial
*              - const uint8_t *a: pointer to input byte array
*                                  (of MLKEM_POLYBYTES bytes)
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_frombytes(poly *r, const uint8_t a[MLKEM_POLYBYTES]) {
    PQCLEAN_MLKEM768_AVX2_nttfrombytes_avx(r->vec, a, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_frommsg
*
...
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_frommsg(poly *restrict r, const uint8_t msg[MLKEM_INDCPA_MSGBYTES]) {
...
/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_tomsg
*
...
*              - poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_tomsg(uint8_t msg[MLKEM_INDCPA_MSGBYTES], const poly *restrict a) {
    unsigned int i;
    uint32_t small;
    __m256i f0, f1, g0, g1;
...
/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_getnoise_eta1
*
...
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_getnoise_eta1(poly *r, const uint8_t seed[MLKEM_SYMBYTES], uint8_t nonce) {
    ALIGNED_UINT8(MLKEM_ETA1 * MLKEM_N / 4 + 32) buf; // +32 bytes as required by PQCLEAN_MLKEM768_AVX2_poly_cbd_eta1
    prf(buf.coeffs, MLKEM_ETA1 * MLKEM_N / 4, seed, nonce);
    PQCLEAN_MLKEM768_AVX2_poly_cbd_eta1(r, buf.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_getnoise_eta2
*
* Description: Sample a polynomial deterministically from a seed and a nonce,
*              with output polynomial close to centered binomial distribution
...
*              - uint8_t nonce: one-byte input nonce
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_getnoise_eta2(poly *r, const uint8_t seed[MLKEM_SYMBYTES], uint8_t nonce) {
    ALIGNED_UINT8(MLKEM_ETA2 * MLKEM_N / 4) buf;
    prf(buf.coeffs, MLKEM_ETA2 * MLKEM_N / 4, seed, nonce);
    PQCLEAN_MLKEM768_AVX2_poly_cbd_eta2(r, buf.vec);
}

#define NOISE_NBLOCKS ((MLKEM_ETA1*MLKEM_N/4+SHAKE256_RATE-1)/SHAKE256_RATE)
void PQCLEAN_MLKEM768_AVX2_poly_getnoise_eta1_4x(poly *r0,
        poly *r1,
        poly *r2,
        poly *r3,
    ...
    PQCLEAN_MLKEM768_AVX2_shake256x4_absorb_once(&state, buf[0].coeffs, buf[1].coeffs, buf[2].coeffs, buf[3].coeffs, 33);
    PQCLEAN_MLKEM768_AVX2_shake256x4_squeezeblocks(buf[0].coeffs, buf[1].coeffs, buf[2].coeffs, buf[3].coeffs, NOISE_NBLOCKS, &state);

    PQCLEAN_MLKEM768_AVX2_poly_cbd_eta1(r0, buf[0].vec);
    PQCLEAN_MLKEM768_AVX2_poly_cbd_eta1(r1, buf[1].vec);
    PQCLEAN_MLKEM768_AVX2_poly_cbd_eta1(r2, buf[2].vec);
    PQCLEAN_MLKEM768_AVX2_poly_cbd_eta1(r3, buf[3].vec);
}


/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_ntt
*
...
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_ntt(poly *r) {
    PQCLEAN_MLKEM768_AVX2_ntt_avx(r->vec, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_invntt_tomont
*
...
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_invntt_tomont(poly *r) {
    PQCLEAN_MLKEM768_AVX2_invntt_avx(r->vec, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

void PQCLEAN_MLKEM768_AVX2_poly_nttunpack(poly *r) {
    PQCLEAN_MLKEM768_AVX2_nttunpack_avx(r->vec, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_basemul_montgomery
*
...
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_basemul_montgomery(poly *r, const poly *a, const poly *b) {
    PQCLEAN_MLKEM768_AVX2_basemul_avx(r->vec, a->vec, b->vec, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_tomont
*
...
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_tomont(poly *r) {
    PQCLEAN_MLKEM768_AVX2_tomont_avx(r->vec, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_reduce
*
...
* Arguments:   - poly *r: pointer to input/output polynomial
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_reduce(poly *r) {
    PQCLEAN_MLKEM768_AVX2_reduce_avx(r->vec, PQCLEAN_MLKEM768_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_add
*
...
*            - const poly *b: pointer to second input polynomial
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_add(poly *r, const poly *a, const poly *b) {
    unsigned int i;
    __m256i f0, f1;
...
/*************************************************
* Name:        PQCLEAN_MLKEM768_AVX2_poly_sub
*
...
*            - const poly *b: pointer to second input polynomial
**************************************************/
void PQCLEAN_MLKEM768_AVX2_poly_sub(poly *r, const poly *a, const poly *b) {
    unsigned int i;
    __m256i f0, f1;
