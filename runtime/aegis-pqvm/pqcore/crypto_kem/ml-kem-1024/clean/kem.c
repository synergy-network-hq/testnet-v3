...
int PQCLEAN_MLKEM1024_CLEAN_crypto_kem_enc_derand(uint8_t *ct,
    ...
    PQCLEAN_MLKEM1024_CLEAN_indcpa_enc(ct, buf, pk, kr + MLKEM_SYMBYTES);
...
int PQCLEAN_MLKEM1024_CLEAN_crypto_kem_dec(uint8_t *ss,
    ...
    /* coins are in kr+MLKEM_SYMBYTES */
    PQCLEAN_MLKEM1024_CLEAN_indcpa_enc(cmp, buf, pk, kr + MLKEM_SYMBYTES);

    fail = PQCLEAN_MLKEM1024_CLEAN_verify(ct, cmp, MLKEM_CIPHERTEXTBYTES);

    /* Compute rejection key */
    rkprf(ss, sk + MLKEM_SECRETKEYBYTES - MLKEM_SYMBYTES, ct);

    /* Copy true key to return buffer if fail is false */
    PQCLEAN_MLKEM1024_CLEAN_cmov(ss, kr, MLKEM_SYMBYTES, (uint8_t) (1 - fail));
