...
#[cfg(feature = "mlkem")]
use pqrust_traits::kem::{Ciphertext as _, SharedSecret as _};
#[cfg(any(feature = "mldsa", feature = "fndsa"))]
use pqrust_traits::sign::DetachedSignature as _;
...
fn mldsa_smoke_tamper_message_or_signature_fails() {
    ...
    sig_bytes[0] ^= 0x01;
    let sig_bad =
        <mldsa::mldsa44::DetachedSignature as pqrust_traits::sign::DetachedSignature>::from_bytes(
            &sig_bytes,
        )
        .unwrap();
    assert!(
...
fn fndsa_smoke_tamper_message_or_signature_fails() {
    ...
    sig_bytes[0] ^= 0x01;
    let sig_bad =
        <fndsa::fndsa512::DetachedSignature as pqrust_traits::sign::DetachedSignature>::from_bytes(
            &sig_bytes,
        )
        .unwrap();
    assert!(
