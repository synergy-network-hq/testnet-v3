...
use crate::p2p::discovery::{
    parse_dnsaddr_multiaddr_to_dial_address, resolve_dns_bootstrap_targets,
};
use crate::p2p::handshake::{
    parse_pqc_algorithm as parse_handshake_pqc_algorithm,
    signing_payload as handshake_pq_signing_payload,
};
...
use crate::p2p::peer_lifecycle::{self as peer_manager, PeerLifecycle};
use crate::p2p::router::{BoundedPeerRouter, RouterAdmissionError, RouterTelemetry};
use crate::p2p::transport::parse_dial_address as parse_bootnode_dial_address;
use crate::p2p::validator_transport_registry::current_validator_transports;
#[cfg(not(test))]
use crate::p2p::validator_transport_registry::{
    refresh_validator_transports, refresh_validator_transports_for_active_validators,
};
use crate::p2p::vpn::{
    chain1266_private_qualification_mode, is_current_validator_vpn_dial_address,
    is_current_validator_vpn_relayer_dial_address, is_validator_vpn_relayer_source_host,
    normalize_validator_address_target, validator_vpn_transport_for_target,
};
#[cfg(test)]
use crate::p2p::vpn::{
    configured_validator_vpn_transport_for_target, is_private_qualification_innernet_dial_address,
...
type PeerLifecycleArc = Arc<Mutex<PeerLifecycle>>;
...
pub struct P2PNetwork {
    ...
    peer_lifecycle: PeerLifecycleArc,
    is_running: Arc<Mutex<bool>>,
    message_router: Arc<BoundedPeerRouter>,
}
...
fn disconnect_peer_entry(
    ...
    peer_manager::clear_peer_session(peer_key);
    drop(gate_guard);
...
fn begin_peer_session(peer_address: &str) -> u64 {
    ...
    peer_manager::begin_peer_session(peer_address)
}
...
fn register_typed_consensus_peer_session(
    ...
    peer_manager::register_typed_consensus_peer_session(peer_address, session_id, identity)
}
...
fn typed_consensus_peer_for_session(
    ...
    peer_manager::typed_consensus_peer_for_session(peer_address, session_id)
}
...
fn current_peer_session_id(peer_address: &str) -> Option<u64> {
    peer_manager::current_peer_session_id(peer_address)
}

fn peer_session_is_current(peer_address: &str, session_id: u64) -> bool {
    peer_manager::peer_session_is_current(peer_address, session_id)
}
...
#[cfg(not(test))]
fn start_validator_transport_refresh_worker(is_running: Arc<Mutex<bool>>, config: NodeConfig) {
    ...
    if !spawn_named_thread("validator-transport-refresh", move || {
        loop {
            ...
            let active_validator_addresses = if local_node_runs_validator_consensus(&config) {
                Some(
                    consensus_membership_validators(VALIDATOR_MANAGER.get_active_validators())
                        .into_iter()
                        .map(|validator| validator.address)
            ...
            let refresh = active_validator_addresses.as_deref().map_or_else(
                refresh_validator_transports,
                refresh_validator_transports_for_active_validators,
            );
            match refresh {
    ...
    peer.handshake_role
        ...
        && peer_connected_endpoint(peer)
            ...
            .is_some_and(|endpoint| match peer.direction {
                ...
                ConnectionDirection::Incoming => is_validator_vpn_relayer_source_host(endpoint),
            })
}
...
impl P2PNetwork {
    pub fn new(blockchain: BlockchainArc, config: &NodeConfig) -> Self {
        P2PNetwork {
            ...
            peer_lifecycle: Arc::new(Mutex::new(PeerLifecycle::new(
                config.network.max_peers as usize,
            ))),
            is_running: Arc::new(Mutex::new(false)),
            message_router: Arc::new(BoundedPeerRouter::production()),
        }
    }

    /// Bounded-router pressure counters for node health and operator diagnostics.
    pub(crate) fn router_telemetry(&self) -> RouterTelemetry {
        self.message_router.telemetry()
    }
    ...
    pub fn start(&mut self, listen_address: &str) {
        ...
        let peer_lifecycle = Arc::clone(&self.peer_lifecycle);
        ...
        *is_running.lock().unwrap() = true;
            ...
            && !chain1266_private_qualification_mode()
        {
            let active_validator_addresses =
                consensus_membership_validators(VALIDATOR_MANAGER.get_active_validators())
                    ...
                    .map(|validator| validator.address)
                    .collect::<Vec<_>>();
            match refresh_validator_transports_for_active_validators(&active_validator_addresses) {
                Ok(refresh) => {
                    info!(
                        "p2p",
                        "Loaded provider-signed validator transport registry covering active validator membership before network start",
                        "generation" => refresh.generation,
                        "active_validator_count" => active_validator_addresses.len() as u64
                    );
                }
                Err(error) => {
                    error!(
            ...
            start_validator_transport_refresh_worker(Arc::clone(&is_running), self.config.clone());
        }
        ...
        let peer_lifecycle_handler = Arc::clone(&self.peer_lifecycle);
        let handler_config = self.config.clone();
        let handler_router = Arc::clone(&self.message_router);
    ...
    pub fn connect_to_peer(&self, address: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let admitted = self
            ...
            .map(|mut lifecycle| {
                lifecycle
                    .begin_outbound(&peer_address, current_timestamp())
                    .is_ok()
            })
            .unwrap_or(false);
        if !admitted {
        ...
        if !reserve_outbound_dial(
            ...
            if let Ok(mut lifecycle) = self.peer_lifecycle.lock() {
                lifecycle.mark_disconnected(&peer_address, current_timestamp(), false);
            }
            return Ok(());
        }
        ...
        let peer_address_for_thread = peer_address.clone();
        ...
        let spawned = spawn_named_thread("p2p-connect-peer", move || {
            match dial_with_timeout(&transport_address, std::time::Duration::from_secs(5)) {
                Ok(stream) => {
                    if let Ok(mut lifecycle) = peer_lifecycle.lock() {
                        lifecycle.mark_connected(&peer_address_for_thread, current_timestamp());
                    }
                    if let Err(e) = handle_outgoing_connection(
                        stream,
                        peer_address_for_thread.clone(),
                        blockchain,
                        connected_peers,
                        peer_state_cache,
                        message_router,
                        ...
                        if let Ok(mut lifecycle) = peer_lifecycle.lock() {
                            lifecycle.mark_disconnected(
                                &peer_address_for_thread,
                                current_timestamp(),
                                true,
                            );
                        }
                        error!("p2p", "Outgoing connection error", "error" => e.to_string());
                    } else if let Ok(mut lifecycle) = peer_lifecycle.lock() {
                        lifecycle.mark_disconnected(
                            &peer_address_for_thread,
                            current_timestamp(),
                            false,
        ...
        if !spawned {
            ...
            if let Ok(mut lifecycle) = self.peer_lifecycle.lock() {
                lifecycle.mark_disconnected(&peer_address, current_timestamp(), true);
            }
        }
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                let peer_lifecycle_clone = Arc::clone(&peer_lifecycle);
                let router_clone = Arc::clone(&message_router);
...
fn dispatch_peer_message(
    ...
    connected_peers: &PeersArc,
    peer_state_cache: &PeerStateCacheArc,
    message_router: &BoundedPeerRouter,
    config: &NodeConfig,
    peer_address: &str,
    session_id: u64,
    message: NetworkMessage,
    ...
    if !bypasses_shared_message_queue(&message) {
        return message_router.try_admit(peer_address, session_id, message);
    }
...
fn handle_incoming_connection(
    ...
    if !peer_lifecycle
        ...
        .map(|mut lifecycle| {
            lifecycle
                .begin_inbound(&peer_address, current_timestamp())
                .is_ok()
        })
        .unwrap_or(false)
    {
        return Err(io::Error::new(
            ...
            "peer lifecycle rejected inbound connection",
        )
        .into());
    }
    if let Ok(mut lifecycle) = peer_lifecycle.lock() {
        lifecycle.mark_connected(&peer_address, current_timestamp());
    }
    ...
    loop {
        match receive_message(&mut reader) {
            Ok(message) => {
                ...
                if let Err(error) = dispatch_peer_message(
                    &blockchain,
                    &connected_peers,
                    &peer_state_cache,
                    ...
                    warn!(
                        "p2p",
                        "Disconnecting peer after bounded-router admission failure",
                        "peer" => peer_address.clone(),
                        "error" => format!("{error:?}")
                    );
                    break;
    ...
    message_router.discard_session(&peer_address, peer_entry_guard.session_id);
    if let Ok(mut lifecycle) = peer_lifecycle.lock() {
        lifecycle.mark_disconnected(&peer_address, current_timestamp(), false);
    }
    Ok(())
...
fn handle_messages(
    ...
    loop {
        let routed = message_router.receive();
        let peer_address = routed.peer_address;
        let session_id = routed.session_id;
        let message = routed.message;
        ...
        if let Ok(mut lifecycle) = peer_lifecycle.lock() {
            lifecycle.mark_activity(&peer_address, current_timestamp());
        }
        ...
        match message {
            NetworkMessage::Handshake {
                ...
                let typed_consensus_peer =
                    match verify_handshake_pq_signature(&handshake_for_verification) {
                ...
                if let Some(identity) = typed_consensus_peer {
                    if let Err(reason) =
                        register_typed_consensus_peer_session(&peer_address, session_id, identity)
                    {
                        warn!(
                            "p2p",
                ...
                let normalized_public_address = if canonicalize_validator_public_address {
                    canonical_validator_public_address(&peer_address, public_address.as_deref())
                } else {
                    public_address
                        .as_deref()
                        .and_then(parse_bootnode_dial_address)
                ...
                if let Ok(mut lifecycle) = peer_lifecycle.lock() {
                    lifecycle.mark_authenticated(&peer_address, current_timestamp());
                }
                ...
                let mut skip_handshake_followup = false;
                {
                    ...
                    if let Some(existing_key) = existing_peer_key.clone() {
                        if existing_key != peer_address {
                            ...
                            let existing_cached_state = peers.get(&existing_key).and_then(|peer| {
                                ...
                                ) = (existing_metadata, new_metadata)
                                {
                                    ...
                                    match duplicate_resolution {
                                        DuplicateResolution::KeepExisting => {
                                            ...
                                            deferred_status_request = current_peer_session_id(
                                                &existing_key,
                                            )
                                            .map(|session_id| (existing_key.clone(), session_id));
                                            warn!(
                                                "p2p",
                ...
                // activation must run through the explicit source-level workflow.
                {
                    ...
                    if config.node.auto_register_validator {
                        let Some(validator_address) = announced_validator_address.clone() else {
                            debug!(
                                "p2p",
                    ...
                    for (address, target_session_id) in targets {
                        match send_peer_message_for_session(
                            ...
                            Ok(true) => {
                                let mut peers = connected_peers.lock().unwrap();
                                if let Some(peer) =
                                    peer_for_session_mut(&mut peers, &address, target_session_id)
                                {
                                    peer.txs_sent += 1;
                                    forwarded_peers += 1;
                                }
            ...
            NetworkMessage::Error { message } if message.starts_with("block-sync-busy:") => {
...
fn dial_peer_async(
    ...
    let peer_lifecycle_for_thread = Arc::clone(&peer_lifecycle);
    let spawned = spawn_named_thread("p2p-discovery-dial", move || {
        if let Some((source_peer, source_session_id)) = source_session.as_ref() {
            if !peer_session_is_current(source_peer, *source_session_id) {
                ...
                if let Ok(mut lifecycle) = peer_lifecycle_for_thread.lock() {
                    lifecycle.mark_disconnected(
                        &cleanup_address_for_thread,
                        current_timestamp(),
                        false,
                    );
                }
        ...
        match dial_with_timeout(&transport_address, std::time::Duration::from_secs(5)) {
            Ok(stream) => {
                ...
                } else if let Ok(mut lifecycle) = peer_lifecycle_for_thread.lock() {
                    lifecycle.mark_disconnected(&peer_address, current_timestamp(), false);
                }
            }
            Err(e) => {
    ...
    if !spawned {
        release_outbound_dial(&dial_registry, &cleanup_address);
        if let Ok(mut lifecycle) = peer_lifecycle.lock() {
            lifecycle.mark_disconnected(&cleanup_address, current_timestamp(), true);
        }
    }
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use crate::p2p::router::BoundedPeerRouter;
    ...
    use super::{
        ...
        configured_seed_server_dial_targets, configured_validator_p2p_dials,
        configured_validator_public_address_map,
        ...
        PEER_WRITE_GATES, PENDING_BLOCKS, POSY_SIMPLIFIED_PROTOCOL_VERSION,
    ...
    fn service_sync_test_guard() -> MutexGuard<'static, ()> {
        ...
        BLOCK_SYNC_APPLY_ACTIVE
            ...
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .clear();
        super::peer_manager::clear_peer_sessions_for_tests();
        PEER_WRITE_GATES
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .clear();
    ...
    fn typed_consensus_identity_binding_is_scoped_to_one_live_peer_session() {
        ...
        super::peer_manager::clear_peer_session(&peer_address);
    }
    ...
    fn service_block_response_releases_apply_slot_after_ordered_apply() {
        ...
        let message_router = BoundedPeerRouter::new(2, 1);
    ...
    fn dispatch_peer_message_keeps_votes_off_the_background_queue() {
        ...
        let router = BoundedPeerRouter::new(2, 1);
        let blockchain = Arc::new(Mutex::new(BlockChain::new()));
        let config = NodeConfig::default();
        let vote = Vote {
            validator_address: "synv1peer-a".to_string(),
