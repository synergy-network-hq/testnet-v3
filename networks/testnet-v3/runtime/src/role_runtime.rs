...
use crate::consensus::dao_governance::SynergyOracle;
...
/// Cross-Chain Nodes may consume verified finality for SXCP work, while the
/// gateway and indexer remain read-only observers.
fn should_start_typed_finality_observer(
    ...
    !config.node.bootstrap_only
        && matches!(
            ...
            Some(NodeRole::CrossChain | NodeRole::RpcGateway | NodeRole::Indexer)
        )
}
...
fn resolved_consensus_runtime_preflight(
    ...
    {
        Ok(ResolvedConsensusMode::PosySimplifiedV3) => Ok(ResolvedConsensusMode::PosySimplifiedV3),
        Ok(ResolvedConsensusMode::CoordinatedRoundRobinV1(_)) => Err(
            "fresh Testnet-v3 consensus refuses coordinated-round-robin runtime selection"
                .to_string(),
        ),
        Err(error) => Err(error),
...
fn start_role_local_services(
    ...
    match profile.role {
        ...
        NodeRole::NetworkAnalytics => {
            active.retain("network-analytics-service".to_string());
        }
        ...
        NodeRole::Indexer => {
            ...
            let Some(runtime_root) = runtime_root else {
                eprintln!(
            "Indexer role requires a runtime root with config/ and bundled synergy-atlas assets."
                );
                return active;
            };

            let Some(atlas_root) = resolve_synergy_atlas_root(&runtime_root) else {
                eprintln!("Indexer role requires synergy-atlas directory near the node runtime.");
                return active;
            };

            if Command::new("node").arg("--version").output().is_err() {
                eprintln!("Indexer role requires Node.js available on PATH.");
                return active;
            }
            ...
            let database_url = match env::var("DATABASE_URL") {
                Ok(value) => value,
                Err(_) => {
                    eprintln!("Indexer role requires DATABASE_URL for Postgres.");
                    return active;
                }
            };
            ...
            let Some(indexer_script) = resolve_node_entrypoint(&indexer_dir) else {
                eprintln!("Indexer role could not find an Atlas indexer entrypoint.");
                return active;
            };
            let Some(backend_script) = resolve_node_entrypoint(&backend_dir) else {
                eprintln!("Indexer role could not find an Atlas backend entrypoint.");
                return active;
            };
...
pub fn run(binary_name: &'static str, expected_profile: Option<&'static RoleProfile>) {
    ...
    match subcommand.as_str() {
        ...
        "start" | "preflight-release" => {
            ...
            // Administration uses a user-owned Unix-domain socket rather than
            // the public RPC/P2P listeners. Both the terminal client and the
            // Node Control Panel consume this exact typed contract.
            #[cfg(unix)]
            let local_admin_api = crate::node_management::start_local_admin_api(
                Some(effective_config_path.to_string_lossy().to_string()),
                data_dir.join("synergy-node-admin.sock"),
            )
            .unwrap_or_else(|error| {
                eprintln!("Local Admin API failed closed: {}", error.message);
                process::exit(1);
            });
            #[cfg(unix)]
            info!(
                ...
                "socket" => local_admin_api.socket_path().display().to_string()
            );
...
mod tests {
    ...
    #[test]
    fn only_cross_chain_gateway_and_indexer_start_non_signing_typed_finality_observer() {
        let config = NodeConfig::default();
        assert!(should_start_typed_finality_observer(
            &config,
            Some(NodeRole::CrossChain.profile())
        ));
        assert!(should_start_typed_finality_observer(
            &config,
    ...
    #[test]
    fn cross_chain_profile_starts_p2p() {
        assert!(role_profile_requires_p2p(NodeRole::CrossChain.profile()));
    }

    #[test]
    fn indexer_profile_starts_p2p_and_sync() {
        let config = NodeConfig::default();
        let profile = NodeRole::Indexer.profile();
