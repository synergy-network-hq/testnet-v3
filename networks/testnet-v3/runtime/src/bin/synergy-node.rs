...
use std::fs;
use synergy_admin_api::{AdminRequest, AdminService};
...
fn run() -> Result<(), String> {
    ...
    match command {
        ...
        "status" | "health" | "readiness" | "doctor" | "validate-config" | "role"
        | "capabilities" => run_management_command(command, &args)?,
...
fn run_management_command(command: &str, args: &[String]) -> Result<(), String> {
    let operation = match command {
        ...
        "validate-config" => ManagementOperation::ConfigurationValidation,
        "role" => ManagementOperation::RoleInspection,
        "capabilities" => ManagementOperation::CapabilityDiscovery,
        _ => return Err(format!("unsupported management command {command}")),
    };
    let request = AdminRequest::new(
        format!("cli-{}-{}", command, current_timestamp()),
        operation,
    );
    #[cfg(unix)]
    let response = if let Some(socket_path) = arg_value(args, "--admin-socket") {
        synergy_admin_api::local::request(socket_path, &request)
            .map_err(|error| format!("{}: {}", error.code, error.message))?
    } else {
        let core =
            synergy_testnet::node_management::NodeManagementCore::new(arg_value(args, "--config"));
        core.handle(request)
    };
    #[cfg(not(unix))]
    let response = {
