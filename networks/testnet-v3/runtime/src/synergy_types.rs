...
mod tests {
    ...
    #[test]
    fn frozen_validator_weight_commitment_preserves_non_uniform_weights() {
        let (mut set, _, _, _, _) = context_fixture(6);
        for (index, validator) in set.validators.iter_mut().enumerate() {
            validator.voting_weight = 100 + index as u64;
        }
        set.validate_unique_validator_and_key_ids()
            .expect("approved non-zero frozen validator weights remain valid");
        assert_ne!(
            set.frozen_bonded_weight_root().unwrap(),
            Hash::zero(),
            "the canonical frozen-weight commitment must retain each approved weight"
        );
    }
