...
simple_struct!(PublicKey, ffi::PQCLEAN_MLKEM512_CLEAN_CRYPTO_PUBLICKEYBYTES);
simple_struct!(SecretKey, ffi::PQCLEAN_MLKEM512_CLEAN_CRYPTO_SECRETKEYBYTES);
simple_struct!(
    Ciphertext,
    ffi::PQCLEAN_MLKEM512_CLEAN_CRYPTO_CIPHERTEXTBYTES
);
simple_struct!(SharedSecret, ffi::PQCLEAN_MLKEM512_CLEAN_CRYPTO_BYTES);

pub const fn public_key_bytes() -> usize {
    ffi::PQCLEAN_MLKEM512_CLEAN_CRYPTO_PUBLICKEYBYTES
}

pub const fn secret_key_bytes() -> usize {
    ffi::PQCLEAN_MLKEM512_CLEAN_CRYPTO_SECRETKEYBYTES
}
...
pub const fn shared_secret_bytes() -> usize {
    ffi::PQCLEAN_MLKEM512_CLEAN_CRYPTO_BYTES
}
...
macro_rules! gen_keypair {
    ($variant:ident) => {{
        ...
        ffi_result("mlkem512_keypair", unsafe {
...
pub fn keypair() -> Result<(PublicKey, SecretKey)> {
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair!(PQCLEAN_MLKEM512_AVX2_crypto_kem_keypair);
        }
    }
...
macro_rules! encap {
    ($variant:ident, $pk:ident) => {{
        ...
        ffi_result("mlkem512_encapsulate", unsafe {
...
pub fn encapsulate(pk: &PublicKey) -> Result<(SharedSecret, Ciphertext)> {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return encap!(PQCLEAN_MLKEM512_AVX2_crypto_kem_enc, pk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return encap!(PQCLEAN_MLKEM512_AARCH64_crypto_kem_enc, pk);
        }
    }
    encap!(PQCLEAN_MLKEM512_CLEAN_crypto_kem_enc, pk)
}

macro_rules! decap {
    ($variant:ident, $ct:ident, $sk:ident) => {{
        let mut ss = SharedSecret::new();
        ffi_result("mlkem512_decapsulate", unsafe {
...
pub fn decapsulate(ct: &Ciphertext, sk: &SecretKey) -> Result<SharedSecret> {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return decap!(PQCLEAN_MLKEM512_AVX2_crypto_kem_dec, ct, sk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return decap!(PQCLEAN_MLKEM512_AARCH64_crypto_kem_dec, ct, sk);
        }
    }
    decap!(PQCLEAN_MLKEM512_CLEAN_crypto_kem_dec, ct, sk)
}
...
mod test {
    ...
    #[test]
    pub fn test_kem() {
        let (pk, sk) = keypair().expect("ML-KEM-512 keypair should succeed");
        let (ss1, ct) = encapsulate(&pk).expect("ML-KEM-512 encapsulation should succeed");
        let ss2 = decapsulate(&ct, &sk).expect("ML-KEM-512 decapsulation should succeed");
