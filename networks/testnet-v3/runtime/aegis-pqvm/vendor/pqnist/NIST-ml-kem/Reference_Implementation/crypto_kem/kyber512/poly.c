...
void poly_compress(uint8_t r[MLKEM_POLYCOMPRESSEDBYTES], poly *a)
{
  ...
  for(i=0;i<MLKEM_N/8;i++) {
    for(j=0;j<8;j++)
      t[j] = ((((uint16_t)a->coeffs[8*i+j] << 4) + MLKEM_Q/2)/MLKEM_Q) & 15;
  ...
  for(i=0;i<MLKEM_N/8;i++) {
    for(j=0;j<8;j++)
      t[j] = ((((uint32_t)a->coeffs[8*i+j] << 5) + MLKEM_Q/2)/MLKEM_Q) & 31;
...
*              - poly *a:      pointer to input polynomial
**************************************************/
void poly_tomsg(uint8_t msg[MLKEM_INDCPA_MSGBYTES], poly *a)
{
  unsigned int i,j;
  uint16_t t;
  ...
  for(i=0;i<MLKEM_N/8;i++) {
    msg[i] = 0;
    for(j=0;j<8;j++) {
      t = ((((uint16_t)a->coeffs[8*i+j] << 1) + MLKEM_Q/2)/MLKEM_Q) & 1;
      msg[i] |= t << j;
    }
  }
}
