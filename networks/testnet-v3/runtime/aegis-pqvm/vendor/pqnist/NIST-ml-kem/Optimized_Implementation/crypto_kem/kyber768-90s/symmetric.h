...
#define mlkem_aes256xof_absorb MLKEM_NAMESPACE(_mlkem_aes256xof_absorb)
void mlkem_aes256xof_absorb(aes256ctr_ctx *state,
                            const uint8_t seed[MLKEM_SYMBYTES],
                            uint8_t x,
                            uint8_t y);

#define mlkem_aes256ctr_prf MLKEM_NAMESPACE(_mlkem_aes256ctr_prf)
...
#define xof_absorb(STATE, SEED, X, Y) \
        mlkem_aes256xof_absorb(STATE, SEED, X, Y)
#define xof_squeezeblocks(OUT, OUTBLOCKS, STATE) \
        aes256ctr_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define prf(OUT, OUTBYTES, KEY, NONCE) \
        mlkem_aes256ctr_prf(OUT, OUTBYTES, KEY, NONCE)
...
#define mlkem_shake128_absorb MLKEM_NAMESPACE(_mlkem_shake128_absorb)
void mlkem_shake128_absorb(keccak_state *s,
                           const uint8_t seed[MLKEM_SYMBYTES],
                           uint8_t x,
                           uint8_t y);

#define mlkem_shake256_prf MLKEM_NAMESPACE(_mlkem_shake256_prf)
