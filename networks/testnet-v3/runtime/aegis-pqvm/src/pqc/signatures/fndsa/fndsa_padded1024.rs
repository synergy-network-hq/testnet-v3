...
//! # Example
//! ```
//! use crate::pqc::signatures::fndsa::fndsa_padded1024::*;
...
simple_struct!(
    PublicKey,
    ffi::PQCLEAN_FNDSAPADDED1024_CLEAN_CRYPTO_PUBLICKEYBYTES
);
simple_struct!(
    SecretKey,
    ffi::PQCLEAN_FNDSAPADDED1024_CLEAN_CRYPTO_SECRETKEYBYTES
);
...
pub struct DetachedSignature(
    ...
    [u8; ffi::PQCLEAN_FNDSAPADDED1024_CLEAN_CRYPTO_BYTES],
    usize,
);
...
impl DetachedSignature {
    fn new() -> Self {
        DetachedSignature([0u8; ffi::PQCLEAN_FNDSAPADDED1024_CLEAN_CRYPTO_BYTES], 0)
    }
}
...
impl primitive::DetachedSignature for DetachedSignature {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let expected = ffi::PQCLEAN_FNDSAPADDED1024_CLEAN_CRYPTO_BYTES;
        if actual > expected {
        ...
        let mut array = [0u8; ffi::PQCLEAN_FNDSAPADDED1024_CLEAN_CRYPTO_BYTES];
...
pub const fn signature_bytes() -> usize {
    ffi::PQCLEAN_FNDSAPADDED1024_CLEAN_CRYPTO_BYTES
}
...
/// Generate a FN-DSA-padded-1024 keypair
pub fn keypair() -> (PublicKey, SecretKey) {
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair!(PQCLEAN_FNDSAPADDED1024_AVX2_crypto_sign_keypair);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        ...
        // the moment
        if true {
            return gen_keypair!(PQCLEAN_FNDSAPADDED1024_AARCH64_crypto_sign_keypair);
        }
    }
    gen_keypair!(PQCLEAN_FNDSAPADDED1024_CLEAN_crypto_sign_keypair)
}
...
pub fn sign(msg: &[u8], sk: &SecretKey) -> SignedMessage {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_signature!(PQCLEAN_FNDSAPADDED1024_AVX2_crypto_sign, msg, sk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return gen_signature!(PQCLEAN_FNDSAPADDED1024_AARCH64_crypto_sign, msg, sk);
        }
    }
    gen_signature!(PQCLEAN_FNDSAPADDED1024_CLEAN_crypto_sign, msg, sk)
}
...
pub fn open(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return open_signed!(PQCLEAN_FNDSAPADDED1024_AVX2_crypto_sign_open, sm, pk);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        if true {
            return open_signed!(PQCLEAN_FNDSAPADDED1024_AARCH64_crypto_sign_open, sm, pk);
        }
    }
    open_signed!(PQCLEAN_FNDSAPADDED1024_CLEAN_crypto_sign_open, sm, pk)
}
...
pub fn detached_sign(msg: &[u8], sk: &SecretKey) -> DetachedSignature {
    ...
    detached_signature!(PQCLEAN_FNDSAPADDED1024_CLEAN_crypto_sign_signature, msg, sk)
}
