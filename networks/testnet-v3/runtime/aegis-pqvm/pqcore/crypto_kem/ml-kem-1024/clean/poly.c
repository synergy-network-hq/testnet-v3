...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_compress(uint8_t r[MLKEM_POLYCOMPRESSEDBYTES], const poly *a) {
    unsigned int i, j;
    int16_t u;
    uint32_t d0;
    uint8_t t[8];
...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_decompress(poly *r, const uint8_t a[MLKEM_POLYCOMPRESSEDBYTES]) {
    size_t i;
...
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_tobytes(uint8_t r[MLKEM_POLYBYTES], const poly *a) {
    size_t i;
    uint16_t t0, t1;
...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_frombytes(poly *r, const uint8_t a[MLKEM_POLYBYTES]) {
    size_t i;
    for (i = 0; i < MLKEM_N / 2; i++) {
...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_frommsg(poly *r, const uint8_t msg[MLKEM_INDCPA_MSGBYTES]) {
    size_t i, j;

    for (i = 0; i < MLKEM_N / 8; i++) {
        for (j = 0; j < 8; j++) {
            r->coeffs[8 * i + j] = 0;
            PQCLEAN_MLKEM1024_CLEAN_cmov_int16(r->coeffs + 8 * i + j, ((MLKEM_Q + 1) / 2), (msg[i] >> j) & 1);
        }
    }
}
...
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_tomsg(uint8_t msg[MLKEM_INDCPA_MSGBYTES], const poly *a) {
    unsigned int i, j;
    uint32_t t;
...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_getnoise_eta1(poly *r, const uint8_t seed[MLKEM_SYMBYTES], uint8_t nonce) {
...
*              - uint8_t nonce: one-byte input nonce
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_poly_getnoise_eta2(poly *r, const uint8_t seed[MLKEM_SYMBYTES], uint8_t nonce) {
