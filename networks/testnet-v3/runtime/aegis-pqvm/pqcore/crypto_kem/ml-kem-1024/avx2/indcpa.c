...
*              by PQCLEAN_MLKEM1024_AVX2_polyvec_reduce().
*
...
*              by PQCLEAN_MLKEM1024_AVX2_polyvec_reduce() and PQCLEAN_MLKEM1024_AVX2_poly_reduce(), respectively.
*
...
static void pack_ciphertext(uint8_t r[MLKEM_INDCPA_BYTES], polyvec *b, poly *v) {
    ...
    PQCLEAN_MLKEM1024_AVX2_poly_compress(r + MLKEM_POLYVECCOMPRESSEDBYTES, v);
}
...
static void unpack_ciphertext(polyvec *b, poly *v, const uint8_t c[MLKEM_INDCPA_BYTES]) {
    ...
    PQCLEAN_MLKEM1024_AVX2_poly_decompress(v, c + MLKEM_POLYVECCOMPRESSEDBYTES);
}
...
/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_gen_matrix
*
...
void PQCLEAN_MLKEM1024_AVX2_gen_matrix(polyvec *a, const uint8_t seed[32], int transposed) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        while (ctr0 < MLKEM_N || ctr1 < MLKEM_N || ctr2 < MLKEM_N || ctr3 < MLKEM_N) {
            ...
            ctr0 += rej_uniform(a[i].vec[0].coeffs + ctr0, MLKEM_N - ctr0, buf[0].coeffs, SHAKE128_RATE);
            ctr1 += rej_uniform(a[i].vec[1].coeffs + ctr1, MLKEM_N - ctr1, buf[1].coeffs, SHAKE128_RATE);
            ctr2 += rej_uniform(a[i].vec[2].coeffs + ctr2, MLKEM_N - ctr2, buf[2].coeffs, SHAKE128_RATE);
            ctr3 += rej_uniform(a[i].vec[3].coeffs + ctr3, MLKEM_N - ctr3, buf[3].coeffs, SHAKE128_RATE);
        }
...
/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_indcpa_keypair_derand
*
...
**************************************************/
void PQCLEAN_MLKEM1024_AVX2_indcpa_keypair_derand(uint8_t pk[MLKEM_INDCPA_PUBLICKEYBYTES],
...
/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_indcpa_enc
*
...
**************************************************/
void PQCLEAN_MLKEM1024_AVX2_indcpa_enc(uint8_t c[MLKEM_INDCPA_BYTES],
...
/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_indcpa_dec
*
...
**************************************************/
void PQCLEAN_MLKEM1024_AVX2_indcpa_dec(uint8_t m[MLKEM_INDCPA_MSGBYTES],
