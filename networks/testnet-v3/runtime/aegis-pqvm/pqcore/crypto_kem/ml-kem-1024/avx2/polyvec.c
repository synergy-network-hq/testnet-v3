...
**************************************************/
void PQCLEAN_MLKEM1024_AVX2_polyvec_compress(uint8_t r[MLKEM_POLYVECCOMPRESSEDBYTES + 2], const polyvec *a) {
    size_t i;
...
**************************************************/
void PQCLEAN_MLKEM1024_AVX2_polyvec_decompress(polyvec *r, const uint8_t a[MLKEM_POLYVECCOMPRESSEDBYTES + 12]) {
    size_t i;
...
*              - polyvec *a: pointer to input vector of polynomials
**************************************************/
void PQCLEAN_MLKEM1024_AVX2_polyvec_tobytes(uint8_t r[MLKEM_POLYVECBYTES], const polyvec *a) {
    size_t i;
    for (i = 0; i < MLKEM_K; i++) {
        PQCLEAN_MLKEM1024_AVX2_poly_tobytes(r + i * MLKEM_POLYBYTES, &a->vec[i]);
    }
}
...
**************************************************/
void PQCLEAN_MLKEM1024_AVX2_polyvec_frombytes(polyvec *r, const uint8_t a[MLKEM_POLYVECBYTES]) {
    size_t i;
    for (i = 0; i < MLKEM_K; i++) {
        PQCLEAN_MLKEM1024_AVX2_poly_frombytes(&r->vec[i], a + i * MLKEM_POLYBYTES);
    }
}
