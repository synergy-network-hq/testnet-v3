...
//!
//! Read and mutating operations share one typed protocol. Mutations require
//! explicit authorization, a durable idempotency key, and a corresponding
//! runtime owner; clients never infer authority from socket reachability.

use serde::{Deserialize, Serialize};
use serde_json::Value;
use synergy_node_core::{ManagementOperation, MANAGEMENT_SCHEMA_VERSION};

use operations::AdminOperation;
...
impl AdminRequest {
    pub fn new(request_id: impl Into<String>, operation: ManagementOperation) -> Self {
        Self {
            ...
            operation: AdminOperation::Read(operation),
        }
    }

    pub fn query(request_id: impl Into<String>, operation: AdminOperation) -> Self {
        Self {
            schema_version: MANAGEMENT_SCHEMA_VERSION,
            request_id: request_id.into(),
            idempotency_key: None,
    ...
    pub fn validate(&self) -> Result<(), AdminError> {
        ...
        if self.request_id.trim().is_empty() || self.request_id.len() > 256 {
            return Err(AdminError::invalid_request("invalid request ID"));
        }
        ...
        if self.operation.is_mutating()
            && self
                .idempotency_key
                .as_ref()
                .is_none_or(|key| key.trim().is_empty() || key.len() > 256)
        {
            return Err(AdminError::invalid_request(
                "mutating Admin operation requires an idempotency key",
            ));
        }
        Ok(())
    }
...
pub mod local {
    ...
    pub fn request(
        ...
        stream.shutdown(Shutdown::Write).map_err(|error| {
            AdminError::unavailable(format!("finish local Admin API request: {error}"))
        })?;
    ...
    fn handle_connection(mut stream: UnixStream, service: &dyn AdminService) {
        ...
        let read_result = BufReader::new(&mut stream)
            .take((MAX_REQUEST_BYTES + 1) as u64)
            .read_to_end(&mut request_text);
        let response = match read_result {
            Ok(bytes) if bytes > 0 && request_text.len() <= MAX_REQUEST_BYTES => {
                match serde_json::from_slice::<AdminRequest>(&request_text) {
