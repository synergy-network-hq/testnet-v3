...
fn signed_payload(
    ...
    let signature = Sign::mldsa87().sign(&canonical, secret_key).unwrap();
...
fn valid_deploy() -> (
    ...
    let (public_key, secret_key) = Sign::mldsa87().keygen().unwrap();
    let public_key = SynQPublicKey::new(public_key);
    let signer_address = verifier
        .derive_synq_address(&public_key, AlgorithmId::MlDsa87, NetworkId::testnet())
        .unwrap();
...
#[test]
fn valid_mldsa87_deploy_verifies() {
...
#[test]
fn consensus_mldsa65_is_rejected_in_the_account_domain() {
    let (verifier, context, mut deploy) = valid_deploy();
    deploy.signing_payload.algorithm_id = AlgorithmId::MlDsa65;

    assert_eq!(
        verifier
            .verify_contract_deploy(&deploy, &context)
            .unwrap_err(),
        AegisSynQError::UnsupportedAlgorithm
    );
}
...
fn wrong_signer_fails() {
    let (verifier, context, mut deploy) = valid_deploy();
    let (other_public_key, _) = Sign::mldsa87().keygen().unwrap();
...
#[test]
fn valid_mldsa87_call_verifies() {
