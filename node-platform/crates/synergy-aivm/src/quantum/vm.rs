...
impl QuantumVM {
    ...
    pub fn push_input(&mut self, input: &[u8]) -> Result<(), VMError> {
        if !input.is_empty() {
            self.push(Value::Bytes(input.to_vec()))?;
        }
        Ok(())
    }
...
fn missing_pqc_backend_error() -> VMError {
    VMError::CryptoError(
        "cryptographic VM opcode requires an explicitly injected Aegis provider".to_string(),
    )
}
...
fn decapsulate(
    ...
    Err("cryptographic VM opcode requires an explicitly injected Aegis provider".to_string())
}
