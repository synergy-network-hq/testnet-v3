...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_polyvec_compress(uint8_t r[MLKEM_POLYVECCOMPRESSEDBYTES], const polyvec *a) {
    unsigned int i, j, k;
    uint64_t d0;
...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_polyvec_decompress(polyvec *r, const uint8_t a[MLKEM_POLYVECCOMPRESSEDBYTES]) {
    unsigned int i, j, k;
...
*              - const polyvec *a: pointer to input vector of polynomials
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_polyvec_tobytes(uint8_t r[MLKEM_POLYVECBYTES], const polyvec *a) {
    unsigned int i;
    for (i = 0; i < MLKEM_K; i++) {
        PQCLEAN_MLKEM1024_CLEAN_poly_tobytes(r + i * MLKEM_POLYBYTES, &a->vec[i]);
    }
}
...
**************************************************/
void PQCLEAN_MLKEM1024_CLEAN_polyvec_frombytes(polyvec *r, const uint8_t a[MLKEM_POLYVECBYTES]) {
    unsigned int i;
    for (i = 0; i < MLKEM_K; i++) {
        PQCLEAN_MLKEM1024_CLEAN_poly_frombytes(&r->vec[i], a + i * MLKEM_POLYBYTES);
    }
}
