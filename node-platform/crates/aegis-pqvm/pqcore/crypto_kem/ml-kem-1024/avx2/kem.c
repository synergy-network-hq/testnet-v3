...
#include <string.h>
/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_crypto_kem_keypair_derand
*
...
* Returns 0 (success)
**************************************************/
int PQCLEAN_MLKEM1024_AVX2_crypto_kem_keypair_derand(uint8_t *pk,
        uint8_t *sk,
        const uint8_t *coins) {
    PQCLEAN_MLKEM1024_AVX2_indcpa_keypair_derand(pk, sk, coins);
...
/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_crypto_kem_keypair
*
* Description: Generates public and private key
*              for CCA-secure MLKEM key encapsulation mechanism
*
...
* Returns 0 (success)
**************************************************/
int PQCLEAN_MLKEM1024_AVX2_crypto_kem_keypair(uint8_t *pk,
        uint8_t *sk) {
    uint8_t coins[2 * MLKEM_SYMBYTES];
    randombytes(coins, 2 * MLKEM_SYMBYTES);
    PQCLEAN_MLKEM1024_AVX2_crypto_kem_keypair_derand(pk, sk, coins);
    return 0;
}

/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_crypto_kem_enc_derand
*
...
* Returns 0 (success)
**************************************************/
int PQCLEAN_MLKEM1024_AVX2_crypto_kem_enc_derand(uint8_t *ct,
        uint8_t *ss,
        const uint8_t *pk,
        const uint8_t *coins) {
    ...
    PQCLEAN_MLKEM1024_AVX2_indcpa_enc(ct, buf, pk, kr + MLKEM_SYMBYTES);
...
/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_crypto_kem_enc
*
* Description: Generates cipher text and shared
*              secret for given public key
*
...
* Returns 0 (success)
**************************************************/
int PQCLEAN_MLKEM1024_AVX2_crypto_kem_enc(uint8_t *ct,
        uint8_t *ss,
        const uint8_t *pk) {
    uint8_t coins[MLKEM_SYMBYTES];
    randombytes(coins, MLKEM_SYMBYTES);
    PQCLEAN_MLKEM1024_AVX2_crypto_kem_enc_derand(ct, ss, pk, coins);
    return 0;
}

/*************************************************
* Name:        PQCLEAN_MLKEM1024_AVX2_crypto_kem_dec
*
...
**************************************************/
int PQCLEAN_MLKEM1024_AVX2_crypto_kem_dec(uint8_t *ss,
        const uint8_t *ct,
        const uint8_t *sk) {
    int fail;
    uint8_t buf[2 * MLKEM_SYMBYTES];
    ...
    PQCLEAN_MLKEM1024_AVX2_indcpa_dec(buf, ct, sk);
    ...
    /* coins are in kr+MLKEM_SYMBYTES */
    PQCLEAN_MLKEM1024_AVX2_indcpa_enc(cmp, buf, pk, kr + MLKEM_SYMBYTES);

    fail = PQCLEAN_MLKEM1024_AVX2_verify(ct, cmp, MLKEM_CIPHERTEXTBYTES);

    /* Compute rejection key */
    rkprf(ss, sk + MLKEM_SECRETKEYBYTES - MLKEM_SYMBYTES, ct);

    /* Copy true key to return buffer if fail is false */
    PQCLEAN_MLKEM1024_AVX2_cmov(ss, kr, MLKEM_SYMBYTES, (uint8_t) (1 - fail));
