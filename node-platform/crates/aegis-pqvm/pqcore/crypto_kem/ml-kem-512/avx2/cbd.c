...
**************************************************/
static void cbd2(poly *restrict r, const __m256i buf[2 * MLKEM_N / 128]) {
    unsigned int i;
    __m256i f0, f1, f2, f3;
...
*              This function is only needed for MLKEM-512
*
* Arguments:   - poly *r: pointer to output polynomial
*              - const __m256i *buf: pointer to aligned input byte array
**************************************************/
static void cbd3(poly *restrict r, const uint8_t buf[3 * MLKEM_N / 4 + 8]) {
    unsigned int i;
    __m256i f0, f1, f2, f3;
...
void PQCLEAN_MLKEM512_AVX2_poly_cbd_eta1(poly *r, const __m256i buf[MLKEM_ETA1 * MLKEM_N / 128 + 1]) {
    cbd3(r, (uint8_t *)buf);
}

void PQCLEAN_MLKEM512_AVX2_poly_cbd_eta2(poly *r, const __m256i buf[MLKEM_ETA2 * MLKEM_N / 128]) {
    cbd2(r, buf);
}
