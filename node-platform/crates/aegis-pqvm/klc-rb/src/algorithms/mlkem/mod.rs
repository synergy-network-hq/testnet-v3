...
use pqrust_mlkem::{
    mlkem1024::{
        decapsulate as decapsulate1024, encapsulate as encapsulate1024, keypair as keypair1024,
        Ciphertext as Ciphertext1024, PublicKey as PublicKey1024, SecretKey as SecretKey1024,
        SharedSecret as SharedSecret1024,
    },
    mlkem512::{
        decapsulate as decapsulate512, encapsulate as encapsulate512, keypair as keypair512,
        Ciphertext as Ciphertext512, PublicKey as PublicKey512, SecretKey as SecretKey512,
        SharedSecret as SharedSecret512,
    },
    mlkem768::{
        decapsulate as decapsulate768, encapsulate as encapsulate768, keypair as keypair768,
        Ciphertext as Ciphertext768, PublicKey as PublicKey768, SecretKey as SecretKey768,
        SharedSecret as SharedSecret768,
    },
};
...
pub fn mlkem512_keypair() -> (mlkem512PublicKey, mlkem512SecretKey) {
    keypair512().expect("ML-KEM-512 keypair generation failed")
}

pub fn mlkem512_encapsulate(pk: &mlkem512PublicKey) -> (mlkem512Ciphertext, mlkem512SharedSecret) {
    let (ss, ct) = encapsulate512(pk).expect("ML-KEM-512 encapsulation failed");
    (ct, ss)
}

pub fn mlkem512_decapsulate(
    sk: &mlkem512SecretKey,
    ct: &mlkem512Ciphertext,
) -> mlkem512SharedSecret {
    decapsulate512(ct, sk).expect("ML-KEM-512 decapsulation failed")
}

// Similar for 768 and 1024
pub fn mlkem768_keypair() -> (mlkem768PublicKey, mlkem768SecretKey) {
    keypair768().expect("ML-KEM-768 keypair generation failed")
}

pub fn mlkem768_encapsulate(pk: &mlkem768PublicKey) -> (mlkem768Ciphertext, mlkem768SharedSecret) {
    let (ss, ct) = encapsulate768(pk).expect("ML-KEM-768 encapsulation failed");
    (ct, ss)
}
...
pub fn mlkem768_decapsulate(
    ...
    decapsulate768(ct, sk).expect("ML-KEM-768 decapsulation failed")
}

pub fn mlkem1024_keypair() -> (mlkem1024PublicKey, mlkem1024SecretKey) {
    keypair1024().expect("ML-KEM-1024 keypair generation failed")
}

pub fn mlkem1024_encapsulate(
    pk: &mlkem1024PublicKey,
) -> (mlkem1024Ciphertext, mlkem1024SharedSecret) {
    let (ss, ct) = encapsulate1024(pk).expect("ML-KEM-1024 encapsulation failed");
    (ct, ss)
}

pub fn mlkem1024_decapsulate(
    sk: &mlkem1024SecretKey,
    ct: &mlkem1024Ciphertext,
) -> mlkem1024SharedSecret {
    decapsulate1024(ct, sk).expect("ML-KEM-1024 decapsulation failed")
}
...
pub fn mlkem_encapsulate(
    public_key: &[u8],
) -> Result<mlkemEncapsulated, Box<dyn std::error::Error>> {
    use pqrust_traits::kem::PublicKey as _;
    let pk = mlkem768PublicKey::from_bytes(public_key).map_err(|e| {
...
pub fn mlkem_decapsulate(
    ...
    let sk = mlkem768SecretKey::from_bytes(secret_key).map_err(|e| {
        Box::new(std::io::Error::new(std::io::ErrorKind::InvalidData, e))
            as Box<dyn std::error::Error>
    })?;
    let ct = mlkem768Ciphertext::from_bytes(ciphertext).map_err(|e| {
        Box::new(std::io::Error::new(std::io::ErrorKind::InvalidData, e))
            as Box<dyn std::error::Error>
    })?;
...
pub fn mlkem512_encapsulate_native(public_key: &[u8]) -> Result<mlkemEncapsulated, String> {
    ...
    let (ct, ss) = mlkem512_encapsulate(&pk);
    Ok(mlkemEncapsulated {
        ciphertext: ct.as_bytes().to_vec(),
        shared_secret: ss.as_bytes().to_vec(),
    })
...
pub fn mlkem1024_encapsulate_native(public_key: &[u8]) -> Result<mlkemEncapsulated, String> {
    let pk = mlkem1024PublicKey::from_bytes(public_key)
        .map_err(|_| "Invalid public key format".to_string())?;
    let (ct, ss) = mlkem1024_encapsulate(&pk);
    Ok(mlkemEncapsulated {
        ciphertext: ct.as_bytes().to_vec(),
        shared_secret: ss.as_bytes().to_vec(),
    })
