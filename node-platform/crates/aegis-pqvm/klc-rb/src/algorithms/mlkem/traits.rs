...
use crate::mlkem::{mlkem768_decapsulate_native, mlkem768_encapsulate_native};
#[cfg(target_arch = "wasm32")]
use crate::mlkem::{mlkem_decapsulate, mlkem_encapsulate};
use crate::traits::{Algorithm, Kem, KemError};
use std::vec::Vec;
use zeroize::Zeroize;
...
impl Kem for mlkem768 {
    ...
    fn keygen() -> Result<(Self::PublicKey, Self::SecretKey), KemError> {
        ...
        Ok((
            mlkemPublicKey(keypair.public_key()),
            mlkemSecretKey(keypair.secret_key()),
        ))
    }
