use pqrust_mldsa::{
    mldsa44::{
        detached_sign as sign44, keypair as keypair44, verify_detached_signature as verify44,
        DetachedSignature as DetachedSignature44, PublicKey as PublicKey44,
        SecretKey as SecretKey44,
    },
    mldsa65::{
        detached_sign as sign65, keypair as keypair65, verify_detached_signature as verify65,
        DetachedSignature as DetachedSignature65, PublicKey as PublicKey65,
        SecretKey as SecretKey65,
    },
    mldsa87::{
        detached_sign as sign87, keypair as keypair87, verify_detached_signature as verify87,
        DetachedSignature as DetachedSignature87, PublicKey as PublicKey87,
...
pub fn mldsa44_keygen() -> mldsaKeyPair {
    let (pk, sk) = keypair44().expect("ML-DSA-44 keypair generation failed");
    mldsaKeyPair {
...
pub fn mldsa44_sign(secret_key: &[u8], message: &[u8]) -> Vec<u8> {
    ...
    let signature = sign44(message, &sk).expect("ML-DSA-44 signing failed");
...
pub fn mldsa44_verify(public_key: &[u8], message: &[u8], signature: &[u8]) -> bool {
    ...
    let sig = match DetachedSignature44::from_bytes(signature) {
        Ok(sig) => sig,
        Err(_) => return false,
    };

    verify44(&sig, message, &pk).is_ok()
}

// ML-DSA-65 Functions
#[cfg_attr(feature = "wasm", wasm_bindgen)]
pub fn mldsa65_keygen() -> mldsaKeyPair {
    let (pk, sk) = keypair65().expect("ML-DSA-65 keypair generation failed");
    mldsaKeyPair {
        pk: pk.as_bytes().to_vec(),
        sk: sk.as_bytes().to_vec(),
    }
...
pub fn mldsa65_sign(secret_key: &[u8], message: &[u8]) -> Vec<u8> {
    ...
    let signature = sign65(message, &sk).expect("ML-DSA-65 signing failed");
    signature.as_bytes().to_vec()
}
...
pub fn mldsa65_verify(public_key: &[u8], message: &[u8], signature: &[u8]) -> bool {
    ...
    let sig = match DetachedSignature65::from_bytes(signature) {
        Ok(sig) => sig,
        Err(_) => return false,
    };

    verify65(&sig, message, &pk).is_ok()
}

// ML-DSA-87 Functions
#[cfg_attr(feature = "wasm", wasm_bindgen)]
pub fn mldsa87_keygen() -> mldsaKeyPair {
    let (pk, sk) = keypair87().expect("ML-DSA-87 keypair generation failed");
    mldsaKeyPair {
        pk: pk.as_bytes().to_vec(),
        sk: sk.as_bytes().to_vec(),
    }
...
pub fn mldsa87_sign(secret_key: &[u8], message: &[u8]) -> Vec<u8> {
    ...
    let signature = sign87(message, &sk).expect("ML-DSA-87 signing failed");
    signature.as_bytes().to_vec()
}
...
pub fn mldsa87_verify(public_key: &[u8], message: &[u8], signature: &[u8]) -> bool {
    ...
    let sig = match DetachedSignature87::from_bytes(signature) {
        Ok(sig) => sig,
        Err(_) => return false,
    };

    verify87(&sig, message, &pk).is_ok()
}
...
pub fn mldsa_verify(public_key: &[u8], message: &[u8], signature: &[u8]) -> bool {
    mldsa87_verify(public_key, message, signature)
}
