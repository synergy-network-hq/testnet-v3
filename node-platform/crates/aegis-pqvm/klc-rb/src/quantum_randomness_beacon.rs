...
#[cfg(feature = "mlkem")]
use crate::algorithms::mlkem::{mlkem768_encapsulate, mlkem768_keypair};

#[cfg(feature = "mldsa")]
use crate::algorithms::mldsa::{mldsa87_keygen, mldsa87_sign, mldsa87_verify};

fn os_entropy_or_panic(bytes: usize) -> Vec<u8> {
    let mut output = vec![0u8; bytes];
    getrandom::getrandom(&mut output)
        .unwrap_or_else(|err| panic!("OS CSPRNG unavailable for ML-KEM entropy recovery: {err}"));
    output
}
...
impl QuantumBeacon {
    ...
    pub fn new() -> Self {
        ...
        #[cfg(feature = "mldsa")]
        {
            ...
            beacon.signing_keypair = Some((keypair.public_key(), keypair.secret_key()));
        }
        ...
        beacon
            .previous_output
            .copy_from_slice(genesis_hash.as_bytes());
