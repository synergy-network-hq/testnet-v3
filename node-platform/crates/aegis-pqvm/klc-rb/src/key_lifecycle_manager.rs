...
impl KeyLifecycleManager {
    ...
    pub fn new() -> Self {
        ...
        #[cfg(feature = "mldsa")]
        {
            ...
            manager.witness_signing_keypair = Some((keypair.public_key(), keypair.secret_key()));
        }
    ...
    pub fn check_rotation_needed(&self, key_id: KeyId) -> Result<bool, String> {
        ...
        Ok(metadata.usage_count >= policy.max_operations || age >= policy.max_lifetime_seconds)
    }
    ...
    pub fn schedule_rotation(&mut self, key_id: KeyId, reason: &str) -> Result<KeyId, String> {
        ...
        let policy_id = metadata.policy_id.clone();
        let new_key_id = self.generate_key(&policy_id)?;
        let scheduled_at = current_timestamp() + 60;

        let old_metadata = self.keys.get_mut(&key_id).unwrap();
        old_metadata.state = KeyState::RotationScheduled {
            at_timestamp: scheduled_at,
        };
    ...
    pub fn verify_destruction_proof(&self, proof: &ProofOfDestruction) -> bool {
        ...
        let statement = format!(
            ...
            proof.key_id.0, proof.timestamp, proof.method
        );
...
fn generate_random_u64() -> u64 {
    let bytes = blake3::hash(b"random_seed").as_bytes()[..8]
        .try_into()
        .unwrap();
