...
void poly_frommsg(poly * restrict r,
                  const uint8_t msg[MLKEM_INDCPA_MSGBYTES])
{
...
*              - poly *a:      pointer to input polynomial
**************************************************/
void poly_tomsg(uint8_t msg[MLKEM_INDCPA_MSGBYTES], poly * restrict a)
{
  unsigned int i;
  uint32_t small;
  __m256i f0, f1, g0, g1;
  const __m256i hqs = _mm256_set1_epi16((MLKEM_Q - 1)/2);
  const __m256i hhqs = _mm256_set1_epi16((MLKEM_Q - 5)/4);
