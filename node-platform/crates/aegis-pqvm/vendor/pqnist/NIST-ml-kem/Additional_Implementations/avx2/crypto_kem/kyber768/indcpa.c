                      ...
                      const uint8_t packedpk[MLKEM_INDCPA_PUBLICKEYBYTES])
{
  ...
  for(i=0;i<MLKEM_SYMBYTES;i++)
    seed[i] = packedpk[i+MLKEM_POLYVECBYTES];
}
...
**************************************************/
static void pack_ciphertext(uint8_t r[MLKEM_INDCPA_BYTES],
                            polyvec *b,
                            poly *v)
{
...
static void unpack_ciphertext(polyvec *b,
                              poly *v,
                              const uint8_t c[MLKEM_INDCPA_BYTES+6])
{
...
void gen_matrix(polyvec *a, const uint8_t seed[32], int transposed)
{
  ...
  if(transposed) {
    buf[0][MLKEM_SYMBYTES+0] = 0;
    buf[0][MLKEM_SYMBYTES+1] = 0;
    buf[1][MLKEM_SYMBYTES+0] = 0;
    buf[1][MLKEM_SYMBYTES+1] = 1;
    buf[2][MLKEM_SYMBYTES+0] = 1;
    buf[2][MLKEM_SYMBYTES+1] = 0;
    buf[3][MLKEM_SYMBYTES+0] = 1;
    buf[3][MLKEM_SYMBYTES+1] = 1;
  }
  else {
    buf[0][MLKEM_SYMBYTES+0] = 0;
    buf[0][MLKEM_SYMBYTES+1] = 0;
    buf[1][MLKEM_SYMBYTES+0] = 1;
    buf[1][MLKEM_SYMBYTES+1] = 0;
    buf[2][MLKEM_SYMBYTES+0] = 0;
    buf[2][MLKEM_SYMBYTES+1] = 1;
    buf[3][MLKEM_SYMBYTES+0] = 1;
    buf[3][MLKEM_SYMBYTES+1] = 1;
  }

  shake128x4_absorb(&state, buf[0], buf[1], buf[2], buf[3], MLKEM_SYMBYTES+2);
  ...
  while(ctr0 < MLKEM_N || ctr1 < MLKEM_N || ctr2 < MLKEM_N || ctr3 < MLKEM_N) {
    ...
    ctr0 += rej_uniform(a[0].vec[0].coeffs + ctr0, MLKEM_N - ctr0, buf[0],
                        XOF_BLOCKBYTES);
    ctr1 += rej_uniform(a[0].vec[1].coeffs + ctr1, MLKEM_N - ctr1, buf[1],
                        XOF_BLOCKBYTES);
    ctr2 += rej_uniform(a[1].vec[0].coeffs + ctr2, MLKEM_N - ctr2, buf[2],
                        XOF_BLOCKBYTES);
    ctr3 += rej_uniform(a[1].vec[1].coeffs + ctr3, MLKEM_N - ctr3, buf[3],
                        XOF_BLOCKBYTES);
  }
...
void gen_matrix(polyvec *a, const uint8_t seed[32], int transposed)
{
  ...
  if(transposed) {
    ...
    buf[1][MLKEM_SYMBYTES+1] = 1;
    buf[2][MLKEM_SYMBYTES+0] = 0;
    buf[2][MLKEM_SYMBYTES+1] = 2;
    buf[3][MLKEM_SYMBYTES+0] = 1;
    buf[3][MLKEM_SYMBYTES+1] = 0;
  }
  else {
    buf[0][MLKEM_SYMBYTES+0] = 0;
    buf[0][MLKEM_SYMBYTES+1] = 0;
    buf[1][MLKEM_SYMBYTES+0] = 1;
    buf[1][MLKEM_SYMBYTES+1] = 0;
    buf[2][MLKEM_SYMBYTES+0] = 2;
    buf[2][MLKEM_SYMBYTES+1] = 0;
    buf[3][MLKEM_SYMBYTES+0] = 0;
    buf[3][MLKEM_SYMBYTES+1] = 1;
  }
  ...
  while(ctr0 < MLKEM_N || ctr1 < MLKEM_N || ctr2 < MLKEM_N || ctr3 < MLKEM_N) {
    ...
    ctr1 += rej_uniform(a[0].vec[1].coeffs + ctr1, MLKEM_N - ctr1, buf[1],
                        XOF_BLOCKBYTES);
    ctr2 += rej_uniform(a[0].vec[2].coeffs + ctr2, MLKEM_N - ctr2, buf[2],
                        XOF_BLOCKBYTES);
    ctr3 += rej_uniform(a[1].vec[0].coeffs + ctr3, MLKEM_N - ctr3, buf[3],
                        XOF_BLOCKBYTES);
  }
  ...
  if(transposed) {
    buf[0][MLKEM_SYMBYTES+0] = 1;
    buf[0][MLKEM_SYMBYTES+1] = 1;
    buf[1][MLKEM_SYMBYTES+0] = 1;
    buf[1][MLKEM_SYMBYTES+1] = 2;
    buf[2][MLKEM_SYMBYTES+0] = 2;
    buf[2][MLKEM_SYMBYTES+1] = 0;
    buf[3][MLKEM_SYMBYTES+0] = 2;
    buf[3][MLKEM_SYMBYTES+1] = 1;
  }
  else {
    buf[0][MLKEM_SYMBYTES+0] = 1;
    buf[0][MLKEM_SYMBYTES+1] = 1;
    buf[1][MLKEM_SYMBYTES+0] = 2;
    buf[1][MLKEM_SYMBYTES+1] = 1;
    buf[2][MLKEM_SYMBYTES+0] = 0;
    buf[2][MLKEM_SYMBYTES+1] = 2;
    buf[3][MLKEM_SYMBYTES+0] = 1;
    buf[3][MLKEM_SYMBYTES+1] = 2;
  }
  ...
  while(ctr0 < MLKEM_N || ctr1 < MLKEM_N || ctr2 < MLKEM_N || ctr3 < MLKEM_N) {
    ...
    ctr0 += rej_uniform(a[1].vec[1].coeffs + ctr0, MLKEM_N - ctr0, buf[0],
                        XOF_BLOCKBYTES);
    ctr1 += rej_uniform(a[1].vec[2].coeffs + ctr1, MLKEM_N - ctr1, buf[1],
                        XOF_BLOCKBYTES);
    ctr2 += rej_uniform(a[2].vec[0].coeffs + ctr2, MLKEM_N - ctr2, buf[2],
                        XOF_BLOCKBYTES);
    ctr3 += rej_uniform(a[2].vec[1].coeffs + ctr3, MLKEM_N - ctr3, buf[3],
                        XOF_BLOCKBYTES);
  }
  ...
  f = _mm256_load_si256((__m256i *)seed);
  _mm256_store_si256((__m256i *)buf[0], f);
  buf[0][MLKEM_SYMBYTES+0] = 2;
  buf[0][MLKEM_SYMBYTES+1] = 2;
  shake128_absorb(&state1x, buf[0], MLKEM_SYMBYTES+2);
  ...
  while(ctr0 < MLKEM_N)
  {
    ...
    ctr0 += rej_uniform(a[2].vec[2].coeffs + ctr0, MLKEM_N - ctr0, buf[0],
                        XOF_BLOCKBYTES);
  }
...
void gen_matrix(polyvec *a, const uint8_t seed[32], int transposed)
{
  ...
  for(i=0;i<4;i++) {
    ...
    if(transposed) {
      buf[0][MLKEM_SYMBYTES+0] = i;
      buf[0][MLKEM_SYMBYTES+1] = 0;
      buf[1][MLKEM_SYMBYTES+0] = i;
      buf[1][MLKEM_SYMBYTES+1] = 1;
      buf[2][MLKEM_SYMBYTES+0] = i;
      buf[2][MLKEM_SYMBYTES+1] = 2;
      buf[3][MLKEM_SYMBYTES+0] = i;
      buf[3][MLKEM_SYMBYTES+1] = 3;
    }
    else {
      buf[0][MLKEM_SYMBYTES+0] = 0;
      buf[0][MLKEM_SYMBYTES+1] = i;
      buf[1][MLKEM_SYMBYTES+0] = 1;
      buf[1][MLKEM_SYMBYTES+1] = i;
      buf[2][MLKEM_SYMBYTES+0] = 2;
      buf[2][MLKEM_SYMBYTES+1] = i;
      buf[3][MLKEM_SYMBYTES+0] = 3;
      buf[3][MLKEM_SYMBYTES+1] = i;
    }
    ...
    while(ctr0 < MLKEM_N || ctr1 < MLKEM_N || ctr2 < MLKEM_N || ctr3 < MLKEM_N) {
      ...
      ctr0 += rej_uniform(a[i].vec[0].coeffs + ctr0, MLKEM_N - ctr0, buf[0],
                          XOF_BLOCKBYTES);
      ctr1 += rej_uniform(a[i].vec[1].coeffs + ctr1, MLKEM_N - ctr1, buf[1],
                          XOF_BLOCKBYTES);
      ctr2 += rej_uniform(a[i].vec[2].coeffs + ctr2, MLKEM_N - ctr2, buf[2],
                          XOF_BLOCKBYTES);
      ctr3 += rej_uniform(a[i].vec[3].coeffs + ctr3, MLKEM_N - ctr3, buf[3],
                          XOF_BLOCKBYTES);
    }
                    ...
                    uint8_t sk[MLKEM_INDCPA_SECRETKEYBYTES])
{
  ...
  hash_g(buf, buf, MLKEM_SYMBYTES);

  gen_a(a, publicseed);

#ifdef MLKEM_90S
#define NBLOCKS ((2*MLKEM_ETA1*32)/AES256CTR_BLOCKBYTES ) /* Assumes divisibility */
  __attribute__((aligned(16)))
  uint64_t nonce = 0;
  aes256ctr_ctx state;
  __attribute__((aligned(32)))
...
*                                      used as seed (of length MLKEM_SYMBYTES)
                ...
                const uint8_t coins[MLKEM_SYMBYTES])
{
  ...
  __attribute__((aligned(32)))
  uint8_t seed[MLKEM_SYMBYTES];
  polyvec sp, pkpv, ep, at[MLKEM_K], bp;
  poly v, k, epp;
