...
macro_rules! gen_keypair {
    ($variant:ident) => {{
        ...
        ffi_result("hqc128_keypair", unsafe {
...
macro_rules! encap {
    ($variant:ident, $pk:ident) => {{
        ...
        ffi_result("hqc128_encapsulate", unsafe {
...
macro_rules! decap {
    ($variant:ident, $ct:ident, $sk:ident) => {{
        let mut ss = SharedSecret::new();
        ffi_result("hqc128_decapsulate", unsafe {
...
mod test {
    ...
    #[test]
    pub fn test_kem() {
        let (pk, sk) = keypair().expect("HQC-128 keypair should succeed");
        let (ss1, ct) = encapsulate(&pk).expect("HQC-128 encapsulation should succeed");
        let ss2 = decapsulate(&ct, &sk).expect("HQC-128 decapsulation should succeed");
