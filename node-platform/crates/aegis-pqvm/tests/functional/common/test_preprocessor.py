...
def legacy_mldsa_source_glob() -> str:
    return "crypto_sign/{}[235]/aarch64/fips202x2.c".format("di" + "lithium")
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                ...
                if (line.startswith('#if') and 
                    not Path(file).match(legacy_mldsa_source_glob()) and
                    not Path(file).match("crypto_kem/mlkem*/aarch64/fips202x2.c") and
