...
def test_boolean(implementation):
    errors = []
    # pycparser's compatibility C headers do not support the Arm headers.
    ...
    for fname in os.listdir(implementation.path()):
        ...
        ast = pycparser.parse_file(
            ...
            cpp_args=[
                ...
                # include compatibility headers for standard C library types
                '-I{}'.format(
                    os.path.join(tdir, 'pycparser/utils/' + 'fa' + 'ke_libc_include')),
            ]
        )
