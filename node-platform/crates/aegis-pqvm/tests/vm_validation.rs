...
#[test]
fn synergy_dispatch_rejects_empty_payload() {
    let result = SynergyIntegration::dispatch_call(&[]);
    assert!(result.is_err());
}

#[test]
fn synergy_gas_cost_rejects_empty_payload() {
    let result = SynergyIntegration::synergy_gas_cost(&[]);
    assert!(result.is_err());
}

#[test]
fn synergy_invoke_rejects_route_payload_mismatch() {
    let call = Call {
        op: Op::FndsaVerifyDetached,
        alg: Alg::Fndsa512,
        args: vec![vec![0u8; 1], vec![0u8; 1], vec![0u8; 1]],
