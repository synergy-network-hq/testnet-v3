...
///
/// Requires a valid Aegis pqvm license to be installed via
/// [`crate::licensing::init`] before the first call.
pub fn dispatch_deterministic(payload: &[u8]) -> Result<Vec<u8>, IntegrationError> {
    let call = decode_call(payload)?;
    // License check: verify algorithm access and op quota.
    let alg_name = licensing::alg_byte_name(call.alg as u8);
    licensing::check_algorithm(alg_name)
