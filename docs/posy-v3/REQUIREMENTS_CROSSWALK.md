...
| Cross-epoch finality and driver boundary | verified-transition finality environment replays prior-tail material and the driver consumes the exact receiver-owned transition when building boundary transactions | WAL commits/reopens across distinct finalized seed and certified parent; role runtime traverses/re-verifies adjacent transitions and exact prior replay inputs; deterministic retry reuses one transaction | Implemented/evidenced except production finalized-execution authority proof |
...
| Five-OS-process autonomous-driver harness | `posy-simplified-five-driver-harness` | every child owns the production driver, timers, real ML-DSA-65, and distinct durable stores; parent is bounded authenticated routing/fault injection only | Implemented/evidenced; named harness gate passes |
...
| ETDAG/protected execution preserved | protected adapter, schedule-neutral coordinator/ingress, and dynamic H+3 producer bind exact finality, public ML-KEM registry, certified input, execution, proposal material, vote, and QC without importing proposer authority | producer 5/7 dynamic quorum, registry canonicality/substitution, restart, ingress routing, adapter replay/tamper, and finalized-permit role selection tests | Implemented/evidenced; live protected execution and registry provisioning qualification open |
| Authenticated validator role-runtime driver | activation selector constructs the initial epoch; finalized protected mode additionally installs durable authority, schedule-neutral ingress, and H+3 producer/egress in one owned lifecycle | applied Genesis selects deferred core; protected startup requires exact registry; auxiliary fatal propagation and cleanup are owned by `FinalizedPosyWorker` | Implemented for initial epoch; later epoch stops only at finalized-execution transition proof; full node qualification open |
...
| Autonomous driver convergence | distinct wire family, authenticated ingress/fanout, bounded state/material sync, retries, timers, and shutdown handling | five production-driver child processes pass 4/5 progress, 3/5 fail-closed, takeover, finality, partition heal, and exact restart; parent supplies transport only | Implemented/evidenced; full role-runtime/socket/node qualification open |
...
- the production executed-transition authority verifier for the non-circular
  v2 transition subject; role-runtime transition-chain and prior-replay loading
  are implemented but stop at that proof boundary;
