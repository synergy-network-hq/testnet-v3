...
| Canonical identity root and address encoding | SNTS-01 v1.3 registry-backed FN-DSA-1024 identity roots and Bech32m derivation; operational ML-DSA-65/87 keys cannot become address roots | canonical Address Engine vectors, registry hash, rejected typed/reserved namespaces, and identity-authorization fail-closed tests are selected by the PR verifier | Implemented/evidenced; current full verifier run open |
| Full SHA3-512 epoch ring | `derive_epoch_leader_ring` | five input orderings produce identical ring | Implemented/evidenced |
| No health/clock/stake leader input or local election | `SimplifiedEpochContext::authorized_proposer`; fresh-chain config invariants reject the retired coordinator mode and any locally configured ring; runtime preflight rejects it before Genesis I/O | divergent process clocks/health derive identical owner; `fresh_testnet_v3_rejects_coordinator_mode_and_local_ring` and `fresh_runtime_rejects_coordinator_mode_before_genesis_io` are selected by the PR verifier | Implemented/evidenced; current full verifier run open |
