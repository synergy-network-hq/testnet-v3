...
| C | PoSy block integration | `runtime/src/consensus/simplified_posy/material.rs`, `runtime/src/consensus/simplified_posy/protected_material.rs`, and a new integration-only test file under `runtime/src/consensus/simplified_posy/` | PRIMARY and B interfaces | Proposal commitment inclusion/validation and VC reveal-authorization adapter | Valid commitment, mismatch rejection, parent/target binding, reveal-gate tests |
